class Dice {
    ...
    rotate(angleX = 0, angleY = 0, angleZ = 0) {
        ...
        dice.angleX = (dice.angleX + angleX) % Math.PI;
        dice.angleY = (dice.angleY + angleY) % Math.PI;
        dice.angleZ = (dice.angleZ + angleZ) % Math.PI;
...
function getVisitorCount() {
    fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${filePath}`, {
        ...
        .then(data => {
            ...
            updateVisitorCount(count, data.sha);
        })
        .catch(error => console.error('Error:', error));
}

// Function to update visitor count in the file
function updateVisitorCount(count, sha) {
    const apiUrl = `https://api.github.com/repos/${owner}/${repo}/contents/${filePath}`;
    const content = count.toString();
    const message = 'Update visitor count';
    ...
    fetch(apiUrl, {
        ...
        body: JSON.stringify({
            message: message,
            content: btoa(content), // Encode count to base64
            sha: sha
        })
    })
        .catch(error => console.error('Error:', error));
